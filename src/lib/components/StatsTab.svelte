<script lang="ts">
	...
	function getNumFoundWords(length: number): number | undefined {
		return $foundWords.lengthFrequencies.has(length) ? $foundWords.lengthFrequencies.get(length) : 0;
	}
</script>

<div class="stats-wrapper">
	<table>
		{#each [...$possibleWords.lengthFrequencies] as [possibleLength, frequency]}
			<tr>
